function initMap() {
    ...
    let infoWindow = new google.maps.InfoWindow({
        content: "Click to get cargo_list!",
        position: seoul,
    });
...
function addMarker(map) {
    var cargo_locations = global_variable_cargo_locations;
    let location_ports = []

    for (i=0; i < cargo_locations.length; i++) {
       var location_port = cargo_locations[i][4]
    ...
    for (i = 0; i < cargo_locations.length; i++) {
        ...
        const cri_port_name = cargo_locations[i][4];
        var titles = new Set();
        for( let j = 0 ; j < cargo_locations.length; j++) {

            if(cri_port_name == cargo_locations[j][4]) {    // ship's position = 4번째인덱스는 port name
                //기준이 되는 항구이름이   현재반복문을 돌고있는 항구의 이름이면 마커 생성
                //한번의 기준이 끝나면 다음 항구이름으로 검사

                var lay_date = cargo_locations[j][2]
                var can_date = cargo_locations[j][3]
                var lay_date2 = lay_date.slice(5,10)
                var can_date2 = can_date.slice(5,10)

                var title = (cargo_locations[j][0]+' '+cargo_locations[j][1]+' '+lay_date2+'  '+can_date2+' '+cargo_locations[j][5])
        ...
        function createMarker(titles){
            new google.maps.Marker({
            position: new google.maps.LatLng(cargo_locations[i][8], cargo_locations[i][9]),
            map: map,
